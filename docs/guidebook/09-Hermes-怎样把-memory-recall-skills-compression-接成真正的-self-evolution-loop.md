...
- 它会在长会话里整理自己的过去
- 它在响应后还会做 background review，回看这轮经验值不值得留下

单看这些模块都成立。

但这篇要回答的，不是“它还有哪些功能”，而是另一个总问题：
...
这种理解不算错，但会把 Hermes 读扁。

关键不在模块数量，而在这些模块有没有接成回流链。Hermes 不是把过去存成分散材料，而是把过去重新变成下一轮还能直接使用的背景、经验、方法和结构。

所以这一篇的任务，不是重复前面八篇，而是把已经拆开的东西重新装回同一个问题里：

> **过去怎样进入现在，现在又怎样变成未来能力？**
...
仅有持久记忆还不够，因为很多经验不是长期背景，而是情境性过去。
...
这些内容会在当前 query 下被召回、聚合、重构，然后重新进场。

所以“过去进入现在”在 Hermes 里分成两层：
...
对 Hermes 来说，当前回合同时也是新经验的生产现场。至少会产生三类材料：
...
所以当前回合在 Hermes 里既是执行现场，也是后续回流的原料产地。
...
很多系统的 memory 到“留档”就结束了；Hermes 多做了一步，把留档接回了下一轮起跑线。

---

## 五、第二条回流，是“过去会话”在需要时重新变成当前经验

session recall 的回流方式和 memory 不一样。它不是默认带着，而是按需调回。
...
这意味着 Hermes 的过去不只是被保存，而是会在需要时重新活过来；并且不是以原文负担的形式回来，而是以重构后的经验形态回来。
...
如果说 memory 和 recall 让 Hermes 的过去不至于白白消失，那么 skill system 让另一种更高价值的东西开始出现：
...
没有这一层，系统虽然能使用已有的 memory / recall / skills / compression，但对“刚刚发生的这一轮”仍然缺一个正式沉淀关口。
...
所以 background review 不是第五个孤立模块，它更像一个分流阀门：把“刚刚发生的任务过程”送进前面几条回流链。

---

## 九、现在可以把整条 self-evolution loop 压成一张低分辨率总链了

如果把前面几篇拆开的东西重新压成一条最小主链，Hermes 基本就是这样运转的：
...
这条链一旦看清，Hermes 的“自我进化”就不再神秘了。它不是模型参数在变，而是系统持续把过去转写成未来还能直接使用的东西。
...
这套增长路径比 model-level 学习慢一些，但更可控、更可解释、更可审查，也更符合工程系统的现实边界。
